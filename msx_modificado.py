...
# 2. Haz click al botón de arriba a la derecha (►)1

# Si no aparece el botón, reinicia la página o cambia de navegador.

#para iniciar playit pon ./playit-linux-amd64
...
def O():
	A=N()
	if A==I:return
	elif A.split(F)[-1]=='msx':
		if B.name!=E:
			H('Iniciando Playit...')
			B.system('pkill playit-linux-amd64 > /dev/null 2>&1')
			if B.path.exists('./playit-linux-amd64'):
				B.system('chmod +x ./playit-linux-amd64')
				B.system('nohup ./playit-linux-amd64 > playit.log 2>&1 &')
				time.sleep(5)
		B.system(f"chmod +x {A} && ./{A}")
	elif A.split(F)[-1]=='exe':
		B.system(f"start /D {C} {C}\\{A} && exit")
	else:
		B.system(f"python3 {A}")
O()
